...
public:
    ...
    void setUpPointLight(Shader& lightingShader)
    {
        ...
        else if (lightNumber == 4)
        {
            ...
            // Apply a 180-degree rotation around the desired axis (e.g., the y-axis here)
            glm::mat4 rotationMatrix = glm::rotate(identityMatrix, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f));

            // Transform the light's position
            glm::vec4 rotatedPosition = rotationMatrix * glm::vec4(position, 1.0f);

            // Update the shader with the rotated position
            lightingShader.setVec3("pointLights[3].position", glm::vec3(rotatedPosition));
